...
function FAQItem({ question, answer }) {
  ...
  return (
    <button
      ...
      className="w-full text-left bg-slate-900/60 border border-green-500/10 rounded-xl p-5 hover:border-amber-400/20 transition-colors"
    >
      <div className="flex items-center justify-between gap-4">
        <h3 className="font-semibold text-sm text-white">{question}</h3>
        <ChevronDown
          className={`w-4 h-4 text-amber-400 shrink-0 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        />
      </div>
      {open && (
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <nav className="flex items-center gap-4 px-6 py-4 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-2 ml-2">
          <div className="w-6 h-6 rounded bg-gradient-to-br from-green-500 to-amber-400 flex items-center justify-center">
      ...
      <div className="max-w-4xl mx-auto px-6 pb-20">
        <h2 className="text-sm font-semibold text-amber-400/70 uppercase tracking-widest mb-8 text-center">Features</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
          {features.map((feature) => {
            ...
            return (
              <div
                key={feature.title}
                className="bg-slate-900 border border-green-500/15 rounded-xl p-5 hover:border-amber-400/30 transition-colors"
              >
      ...
      <div className="max-w-2xl mx-auto px-6 pb-20">
        <h2 className="text-sm font-semibold text-amber-400/70 uppercase tracking-widest mb-8 text-center">Frequently Asked Questions</h2>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                <input
                  ...
                  className="w-full bg-green-950/30 border border-green-500/20 rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-400 transition-colors"
                />
                <input
                  type="email"
                  placeholder="Your email"
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-gradient-to-r from-green-600 to-amber-500 hover:from-green-700 hover:to-amber-600 disabled:opacity-50 text-white font-semibold rounded-lg py-2.5 text-sm transition-colors"
                >
